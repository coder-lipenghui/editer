...
package morn.core.components {
	...
	public class Button extends Component implements ISelect {
		...
		protected var _labelDirecation:String = "horizontal";
		...
		protected function changeLabelSize():void {
			exeCallLater(changeClips);
			if (_labelDirecation=="vertical") 
			{
				_btnLabel.width = ObjectUtils.getTextField(_btnLabel.format).width;
				_btnLabel.height =height - _labelMargin[1] - _labelMargin[3];
				_btnLabel.x = (width-_btnLabel.width)*0.5+_labelMargin[0]-_labelMargin[2];
				_btnLabel.y = (height - _btnLabel.height) * 0.5 + _labelMargin[1];
			}else {
		...
		public function set isHtml(value:Boolean):void 
		{
			...
			if (_isHtml!=value) 
			{
				_btnLabel.isHtml = _isHtml;
			}
		}
		
		public function get labelDirecation():String 
		{
			return _labelDirecation;
		}
		
		public function set labelDirecation(value:String):void 
		{
			_labelDirecation = value;
			if (_labelDirecation==Tab.VERTICAL) 
			{
				var temp:Array = _btnLabel.text.split("");
				label = temp.join("\n");
				temp = null;
			}
			callLater(changeLabelSize);
