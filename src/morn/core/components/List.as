...
package morn.core.components {
	...
	public class List extends Box implements IRender, IItem {
		...
		protected var _rightClickHandler:Handler;
		...
		protected function changeCells():void {
			if (_itemRender) {
				...
				for each (var cell:Box in _cells) {
					...
					cell.removeEventListener(MouseEvent.RIGHT_CLICK, onCellMouse);
					cell.remove();
				}
				_cells.length = 0;
				//获取滚动条
		...
		protected function addCell(cell:Box):void {
			...
			cell.addEventListener(MouseEvent.RIGHT_CLICK, onCellMouse);
			cell.addEventListener(MouseEvent.DOUBLE_CLICK, onCellMouse);
			_cells.push(cell);
		}
		...
		protected function onCellMouse(e:MouseEvent):void {
			...
			if (e.type==MouseEvent.RIGHT_CLICK) 
			{
				...
				{
					_rightClickHandler.executeWith([index]);
				}
			}else
			{
		...
		public function get rightClickHandler():Handler 
		{
			return _rightClickHandler;
		}
		
		public function set rightClickHandler(value:Handler):void 
		{
			_rightClickHandler = value;
		}
