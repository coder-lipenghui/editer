package editor.manager
{
	...
	public class AnimateManager 
	{
		...
		{
			var _path:String = ProjectConfig.libraryPath + CatalogManager.instance.getAbsolutePath(catalogId) + "/" + DataManager.library.getPathByExportId(catalogId,resId)+ "/export/" + action+".bin";
		...
		public static function getBinAnimateInfo(path:String):Array 
		{
			...
			if (!file.exists) 
			{
				App.log.error("不存在帧描述信息资源:",file.name,path);
				return null;
			}
			if (file.exists) 
			{
